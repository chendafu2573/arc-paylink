...
TCP 80/443 已对公网开放。Caddy 自动维护 `13-212-95-171.sslip.io` 的 TLS 证书；IP 形式的 HTTP 地址作为代理或 Fake-IP 环境下的兼容入口，直接提供同一套公开测试网静态页面。

HTTP 兼容入口不提供传输加密，只能用于公开测试网演示，禁止输入助记词、私钥或使用真实资金。
